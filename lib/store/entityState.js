...
const useEntityState = create((set) => ({ 
    ...
    refetchOrders: async () => {
        ...
        try {
            const { statusFilter, startDateFilter, endDateFilter, orderIdFilter } = useEntityState.getState();
            const queryParams = new URLSearchParams();
            if (statusFilter) queryParams.append('status', statusFilter);
            if (startDateFilter) queryParams.append('start_date', startDateFilter.toISOString());
            if (endDateFilter) queryParams.append('end_date', endDateFilter.toISOString());
            if (orderIdFilter) queryParams.append('order_id', orderIdFilter);

            const res = await BaseApi.get(process.env.NEXT_PUBLIC_API_URL + "/orders?" + queryParams.toString());
