...
export default function Orders() {
    ...
    const chartData = useMemo(() => {
        const labels = filteredOrders.map(order => helper.dateFormatter(order.created_at));
        const earningsData = filteredOrders.map(order => parseFloat(order.promo_price));
            ...
            <div className="container">
                <div className="bg-[#EFF0F0] text-[#121212] p-[50px] rounded-[15px]">
                    <div className="flex items-center justify-between mb-[30px]">
                        <h1 className={`text-[40px] font-black mb-5 text-black ${montserrat.className}`}>
                            Orders
                        </h1>
                        {orders?.length > 0 && (
                            <CSVLink
                                ...
                                <Flatpickr
                                    ...
                                    onChange={(date) => {
                                        ...
                                        if (endDateFilter && date[0] > endDateFilter) {
                                            ...
                                            alert("Start date cannot be later than end date. End date has been cleared.");
                                        }
                                    }}
                                />
                            </div>
