...
export default function Members() {
  ...
  const { members, isMembersLoading, refetchMembers } = useEntityState(
    (state) => ({
      ...
      <div className="container">
        <div className="bg-[#EFF0F0] text-[#121212] p-[50px] rounded-[15px]">
          ...
          <div>
            <Link
              href="/deleted-users"
              className="hover:underline inline-block mb-3 text-red-700 font-bold"
            >
              View Deleted Users
            </Link>
          </div>
          {isMembersLoading ? (
            ...
            <div>
              {members?.length === 0 && (
                <div className="flex flex-col items-center justify-center">
                  ...
                  <h2 className="text-xl font-bold">
                    No promos available at the moment. Please try again later.
                  </h2>
                </div>
              )}
              {members?.map((member, index) => (
                <div
                  key={index}
                  className={`mb-4 bg-[#fff] hover:shadow-[0_4px_8px_0_rgba(0,0,0,0.2)] hover:border-[#ccc] hover:border-[1px] rounded-[10px] p-[30px] ${
                ...
                >
                  {profile?.user_id == member.user_id && (
                    <span className="bg-green-500 text-[12px] font-bold px-[10px] py-[5px] rounded-[10px] inline-block mb-[15px]">
                      Your account
                    </span>
                  )}
                      ...
                      {member?.profile_picture ? (
                        <>
                          <Image
                            ...
                            className="rounded-full inline-block w-[50px] h-[50px] object-cover"
                            width={50}
                            height={50}
                            alt={member.name}
                          />
                      ...
                      member.role === 3 && (
                        <div className="flex flex-col items-center">
                          <Link
                            title="View QR Code"
                            href={
                              process.env.NEXT_PUBLIC_API_URL + member.qr_code
                            }
                            target="_blank"
                  ...
                  <div className="mt-[60px] flex flex-wrap gap-[15px]">
                    <button
                      ...
                      onClick={() => {
                        modalState.setState({
                          modalOpen: true,
                          editInfo: { id: member.user_id },
                          modalInfo: {
                            id: "edit-member",
                            title: `Edit ${member.name} - ${authService.getRole(
                              member.role
                            )}`,
                          },
                        });
                    ...
                    <button
                      ...
                      onClick={() => {
                        modalState.setState({
                          modalOpen: true,
                          deleteInfo: { id: member.user_id },
                          modalInfo: {
                            id: "soft-delete-member",
