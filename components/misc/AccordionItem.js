  ...
  return (
    <div className="mb-3">
      <div
        ...
        className="accordion-header select-none cursor-pointer flex justify-between bg-[#121212] p-[15px] text-[20px] font-bold text-white"
      >
        <span>
          Order #{item?.order_id}
          <span
            className={`px-[10px] ml-[15px] text-[14px] rounded-[5px] pb-[2px] text-black inline-block ${statusBgClass}`}
          >
            {item?.order_status}
          </span>
        </span>
        <svg
          ...
          strokeWidth={1.5}
          stroke="currentColor"
          className={`size-6 transition ${isOpen ? "rotate-180" : ""}`}
        >
          <path
            strokeLinecap="round"
          ...
          <table className="w-full order-detail-table">
            <thead>
              <tr className={`${color ? `bg-[${color}]` : "bg-[#dcdcdc]"}`}>
                <th className="py-[5px] border-r-[1px] border-[#ccc]">
                  Availed Promo
                </th>
            ...
            <tbody className="bg-white">
              <tr className={`text-center`} style={{ color: color || "" }}>
                ...
                <td className="p-[15px]">
                  {helper.priceFormatter(item?.promo_price)}
                </td>
                <td className="p-[15px]">
                  <span
                    className={`text-black px-[15px] py-[5px] rounded-[5px] ${
                ...
                {item?.proof && (
                  <td className="p-[15px]">
                    <Link
                      ...
                      target="_blank"
                      href={process.env.NEXT_PUBLIC_API_URL + item.proof}
                    >
                      <Image
                        className="object-cover"
                        src={process.env.NEXT_PUBLIC_API_URL + item.proof}
                        alt="Proof"
                        width={150}
                        height={150}
                      />
                ...
                <div className="flex gap-x-[15px]">
                  <div
                    onClick={() => {
                      modalState.setState({
                        modalOpen: true,
                        editInfo: { id: item.order_id },
                        modalInfo: {
                          id: "approve-order",
                          title: `Are you sure you want to approve Order #${item.order_id} ?`,
                        },
                      });
                    }}
                  ...
                  <div
                    onClick={() => {
                      modalState.setState({
                        ...
                        modalInfo: {
                          id: "cancel-order",
                          title: `Are you sure you want to cancel Order #${item.order_id} ?`,
                        },
                      });
                    }}
