...
export default function AddNonMember() {
  ...
  const { refetchNonMembersTransactions } = useEntityState((state) => ({
    refetchNonMembersTransactions: state.refetchNonMembersTransactions,
})); 

  const getPromos = async() => {
    try {
      const res = await BaseApi.get(process.env.NEXT_PUBLIC_API_URL + "/nonmember-promos"); 
      if(res.status === 200) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      const res = await BaseApi.post(process.env.NEXT_PUBLIC_API_URL + "/non-members/transactions", data);
      if (res.status === 200) {
  ...
  return (
    <div>
      <form id="add-subscription" onSubmit={onSubmit}>
        ...
        <div className="form-item mb-[15px]">
          <label className="mb-[5px] block">{`Choose a plan (Optional) `}:</label>
        ...
        <div className="form-item mb-[15px]">
          <label className="mb-[5px] block">Note:</label>
          <textarea id="note" name="note" onChange={onChange} className="form-control min-h-[150px] p-[15px] rounded-[5px] border border-black w-full shadow-[0_0_0_1px(#727272,878787)]">
          </textarea>
        </div>
