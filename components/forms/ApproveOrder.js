...
export default function ApproveOrder() {
    ...
    const onYes = useCallback(async () => {
        try {
            const res = await BaseApi.put(`${process.env.NEXT_PUBLIC_API_URL}/orders/${editInfo.id}/approve`);
    ...
    return(
        <div className="actions mt-[30px] flex flex-wrap gap-[15px]">
            <button className="flex max-w-[100px] px-[30px] items-center justify-center bg-green-500 text-center cursor-pointer text-[20px] font-bold rounded-[6px] py-[10px] text-black text-uppercase w-full" onClick={onYes}>Yes</button>
